export default {
    settings: {
      ...
      scheduling: {
        ...
        accountSchedulingStrategyDefault: '默认策略（粘性会话 + 负载感知）',
        accountSchedulingStrategyRoundRobin: '严格轮询',
        accountSchedulingStrategySystem: '系统默认（跟随上方系统级设置）',
        accountSchedulingStrategyHint: '默认策略下同一会话优先复用上次账号（有利上游提示词缓存命中）；严格轮询下每次请求在可用账号间轮换，不读写会话粘性、不区分账号优先级，切回默认策略后自动恢复粘性。previous_response_id 连续对话仍固定原账号。',
        platformSchedulingStrategyTitle: '平台级调度策略',
        platformSchedulingStrategyHint: '平台级策略优先于上方系统级策略；选择「系统默认」的平台跟随系统级设置。可按平台单独强制默认策略或严格轮询。',
